...
LiquidSimulator::LiquidSimulator(TileMap* map) : tileMap(map), isWaterDirty(false) {
    ...
    lavaTex = LoadTexture("assets/sprites/lava/Lava.png"); // No chroma key for the solid lava body!
...
void LiquidSimulator::update(float dt) {
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            if (hasLiquid[y][x]) {
                ...
                bool leftEmpty = (x - 1 >= 0) && !tileMap->isSolid(x - 1, y) && !hasLiquid[y][x - 1];
                bool rightEmpty = (x + 1 < width) && !tileMap->isSolid(x + 1, y) && !hasLiquid[y][x + 1];
                bool bottomEmpty = (y + 1 < height) && !tileMap->isSolid(x, y + 1) && !hasLiquid[y + 1][x];
    ...
    for (auto& pos : toDestroy) {
        ...
        if (typeGrid[y][x] == LiquidType::LAVA && GetRandomValue(1, 100) <= 35) {
            ...
            EventBus::getInstance()->publish(EventType::EVENT_SPAWN_LAVA_DRIP, dripData);
        }
...
void LiquidSimulator::addLiquid(int gx, int gy, uint8_t amount, LiquidType type) {
    ...
    while (gy >= 0 && hasLiquid[gy][gx]) {
        // If the block above is solid, we can't stack anymore (volume is lost/compressed)
    ...
    if (gy < 0 || tileMap->isSolid(gx, gy)) return;
...
bool LiquidSimulator::hasLiquidAt(int gx, int gy) const {
    if (!tileMap->isInBounds(gx, gy)) return false;
...
bool LiquidSimulator::isWaterAt(Rectangle rect) const {
    ...
    int endX = std::min(width - 1, (int)((rect.x + rect.width - 0.1f) / tileSize));
    int endY = std::min(height - 1, (int)((rect.y + rect.height - 0.1f) / tileSize));
    
    for (int y = startY; y <= endY; y++) {
        for (int x = startX; x <= endX; x++) {
            if (hasLiquid[y][x] && typeGrid[y][x] == LiquidType::WATER) return true;
        }
    }
    return false;
}
...
bool LiquidSimulator::isLavaAt(Rectangle rect) const {
    ...
    for (int y = startY; y <= endY; y++) {
        for (int x = startX; x <= endX; x++) {
            if (hasLiquid[y][x] && typeGrid[y][x] == LiquidType::LAVA) return true;
        }
    }
    return false;
}
                ...
                if (y == 0 || !hasLiquid[y - 1][x]) {
                    ...
                    if (dx * dx + dy * dy <= maxDistSq) {
                        ...
                        if (spurtTimer[y][x] <= 0.0f) {
                            ...
                            data.vy = -100.0f * (float)GetRandomValue(3, 6); // -300 to -600 (simulate -rand(1,4))
                            
                            // Publish event
                            EventBus::getInstance()->publish(EventType::EVENT_SPAWN_FLAME, data);
...
void LiquidSimulator::applyFloodedFloorModifier(int bottomRows, LiquidType type) {
